...
def evaluation(args, **kwargs):
    ...
    if args.model_pt is not None:
        if args.model_pt.endswith('ckpt'):
            model = LightningPLM.load_from_checkpoint(checkpoint_path=pjoin('model_ckpt', args.model_pt), hparams=args)
        elif args.model_pt.endswith('bin'):
            state_dict = torch.load(pjoin('model_ckpt', args.model_pt), map_location=device)
    ...
    with torch.no_grad():
        for row in test_data.iterrows():
            ...
            label = int(row[1]['emotion-type'][1]) - 1
